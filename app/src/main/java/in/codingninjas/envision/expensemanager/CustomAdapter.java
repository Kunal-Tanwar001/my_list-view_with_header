...
public class CustomAdapter extends ArrayAdapter {
   ...
   LayoutInflater inflator;
    ...
    public CustomAdapter(Context context, ArrayList<Item> itemsList){
        ...
        inflator=(LayoutInflater)mContext.getSystemService(mContext.LAYOUT_INFLATER_SERVICE);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        View output=convertView;
            ...
            if(itemsList.get(position).getItemType()==0) {
                if (output == null) {
                    output = inflator.inflate(R.layout.expense_row_layout, parent, false);
                    TextView header = output.findViewById(R.id.headerTitleTextView);

                    HeaderItemViewHolder hr = new HeaderItemViewHolder(header);
                    output.setTag(hr);
                }
                HeaderItemViewHolder hr=(HeaderItemViewHolder)output.getTag();
                HeaderItem hi = (HeaderItem) itemsList.get(position);
                hr.headerTitleTextView.setText(hi.getHeaderTitle());
                output.setTag(hr);
                return output;
            }
            else{
